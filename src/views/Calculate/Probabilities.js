function ForgotPass () {
  ...
  fetch('https://api-data-statistic.herokuapp.com/auth/forgot_password', requestInfo)
    .then(response => {
      if (response.ok) {
...
function RetornaArea(x) {
  ...
  return tabelaDisNormal[l][c];
};

function DesvioPadrao(valores, media) {
  let soma = 0;
  for (let i = 0; i < valores.length; i++) {
    soma = soma + ((parseFloat(valores[i]) - media) ** 2);
  }
  return Math.sqrt((soma / valores.length));
}

function Media(valores) {
  return (parseFloat((valores.reduce(function (anterior, atual) {
    return parseFloat(anterior) + parseFloat(atual);
  })) / valores.length));
}

function Normal(media, desv, min, max, opt) {
  let res;
  let negIni = false;
  let negFin = false;
  let zFinal = '';
  let areaFinal;
  let areaTotal = 0.5;
  let Zin = RetornaZ(min, media, desv);
  if (Zin < 0) {
    negIni = true;
    Zin *= -1;
  }
  ...
  let areaInicial = RetornaArea(Zin);
  if (opt === 2) {
    zFinal = RetornaZ(max, media, desv);
    if (zFinal < 0) {
      negFin = true;
      zFinal *= -1;
    }
  ...
  switch (opt) { // 1 - Menor que;  2 - Entre;  3 - Maior que
    case 1:
      if (negIni) {
...
function Uniforme(pontos, Pmin, Pmax, opt) {
  var prob = 0;
  var media = Media(pontos);
  var desvp = DesvioPadrao(pontos, media);
  var coeficienteDU = parseFloat(((desvp / media) * 100));

  switch (opt) { // 1 - Menor que;  2 - Entre;  3 - Maior que
    case 1:
      prob = ((1 / (pontos[0] - pontos[1])) * (Pmin - pontos[1]) * 100);
      break;
    case 2:
      prob = ((1 / (pontos[1] - pontos[0])) * (Pmax - Pmin) * 100);
      break;
    case 3:
      prob = ((1 / (pontos[1] - pontos[0])) * (pontos[1] - Pmin) * 100);
      break;
    default:
      prob = 0;
  }

  return [prob, media, desvp, coeficienteDU];

}

export { Normal, Uniforme };
